	...
	template: `
		<spartan-block-viewer block="rich-filter" title="Functional Rich filter" id="rich-filter-1">
			...
			<p class="text-muted-foreground max-w-3xl text-sm text-pretty">
				The rich filter block provides users with an intuitive interface for creating complex filter criteria. It allows
				users to add multiple filter fields, each with its own set of operators and input controls. The block supports
				various field types, such as text, number, date, and select. The data is exposed as a reactive stream
			</p>
