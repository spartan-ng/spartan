...
@Component({
	selector: 'spartan-bar-chart-interactive',
	...
	template: `
		<spn-bar-chart class="block h-[250px] w-full" [data]="data" [margin]="margin">
			...
			/>
			<spn-bar [dataKey]="interactiveKey()" [name]="seriesName()" [fill]="barFill()" radius="0" />
			<spn-tooltip>
				<ng-template spnTooltipContent let-state>
					...
					>
						@if (state.label !== undefined && state.label !== null && state.label !== '') {
							<div class="font-medium">{{ formatDate(state.label) }}</div>
						}
...
export class BarChartInteractive {
	readonly interactiveKey = input<'desktop' | 'mobile'>('desktop');
	// shadcn colours desktop with --chart-2 and mobile with --chart-1.
	protected readonly barFill = computed(() =>
		this.interactiveKey() === 'desktop' ? 'var(--chart-2)' : 'var(--chart-1)',
	);
	...
	protected readonly seriesName = computed(() => (this.interactiveKey() === 'desktop' ? 'Desktop' : 'Mobile'));
