...
type AreaDatum = { month: string; desktop: number; mobile: number };
				...
				@if (isBrowser) {
					<hlm-chart-container [config]="chartConfig">
						<vis-xy-container [data]="data" [margin]="{ top: 8, bottom: 20, left: 36, right: 8 }">
							<vis-area [x]="xAccessor" [y]="yAccessors" [color]="colorAccessor" [line]="true" [opacity]="0.4" />
							<vis-axis
								type="x"
								position="bottom"
...
export class ChartAreaStackedPreview {
	...
	public readonly yAccessors: NumericAccessor<AreaDatum>[] = [(d: AreaDatum) => d.desktop, (d: AreaDatum) => d.mobile];
	public readonly colorAccessor = (_data: AreaDatum[], i: number) => 'var(--chart-' + (i + 1) + ')';
