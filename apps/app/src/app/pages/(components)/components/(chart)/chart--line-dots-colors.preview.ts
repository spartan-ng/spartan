...
type LineDatum = { month: string; desktop: number; mobile: number; tablet: number };
...
@Component({
	selector: 'spartan-chart-line-dots-colors-preview',
	imports: [
		HlmCardImports,
		HlmChartImports,
		VisLineModule,
	...
	template: `
		<hlm-card class="w-full">
			<hlm-card-header>
				<h3 hlmCardTitle>Line Chart - Dots Colors</h3>
				...
				@if (isBrowser) {
					<hlm-chart-container [config]="chartConfig">
						<vis-xy-container [data]="data" [margin]="{ top: 8, bottom: 20, left: 36, right: 8 }">
							<vis-line [x]="xAccessor" [y]="yAccessors" [color]="colorAccessor" [lineWidth]="2" />
							<vis-axis
								type="x"
								position="bottom"
...
export class ChartLineDotsColorsPreview {
	...
	public readonly yAccessors: NumericAccessor<LineDatum>[] = [
		(d: LineDatum) => d.desktop,
		(d: LineDatum) => d.mobile,
		(d: LineDatum) => d.tablet,
	];
	public readonly colorAccessor = (_data: typeof chartData, i: number) => 'var(--chart-' + (i + 1) + ')';
	...
	public readonly crosshairTemplate = (d: LineDatum) => {
		const items = [
			...
			{ name: 'Tablet', value: d.tablet, color: 'var(--chart-3)' },
		];
