...
const EXAMPLES_DASHBOARD_SETTINGS_KEY = 'spartan-dashboard-examples';

export const DEFAULT_DASHBOARD_TABLE_COLUMNS = {
	id: true,
	type: true,
	status: true,
	target: true,
...
export class DashboardLocalStorageService {
	private readonly _isBrowser = isPlatformBrowser(inject(PLATFORM_ID));

	private readonly _settings = {
		taskTable: {
			selectedColumns:
				this.getSelectedColumnsSettings(EXAMPLES_DASHBOARD_SETTINGS_KEY) ?? DEFAULT_DASHBOARD_TABLE_COLUMNS,
		},
	};

	saveTaskTableColumns(value: Record<string, boolean>): void {
		this._settings.taskTable.selectedColumns = value;
		this.updateSettings(EXAMPLES_DASHBOARD_SETTINGS_KEY, this._settings.taskTable);
	}
	...
	private getSelectedColumnsSettings(key: string) {
		if (!this._isBrowser) {
			return DEFAULT_DASHBOARD_TABLE_COLUMNS;
		}
