	...
	template: `
		<section spartanMainSection>
			...
			<p class="${hlmP}">
				...
				folder and create a <code class="${hlmCode}">[YOUR_APP_NAME]/src/drizzle/db.ts</code> file with the following content:
			</p>
...
export type NewNote = InferInsertModel<typeof notes>;"
			...
			<p class="${hlmP}">
				We first initialize our Postgres client and pass it to Drizzle.
				We then declare our notes table and make Drizzle aware of all its columns and their respective types. Finally, we
				declare some helper types we will use when retrieving and creating our Notes.
			</p>
			...
			<div class="mb-6 mt-4" hlmAlert>
				...
				<p hlmAlertDesc>
					...
					out of the box is not configured with any kind of transformer. JSON responses do not support certain data types like Date and will change 
					<code class="${hlmCode}">Date</code> to <code class="${hlmCode}">string</code>, which breaks our end-to-end type-safety. 
					We can fix this by changing our
					<code class="${hlmCode}">[YOUR_APP_NAME]/src/trpc-client.ts</code> and <code class="${hlmCode}">[YOUR_APP_NAME]/src/server/trpc/trpc.ts</code>
					file to add transformer and pass in <code class="${hlmCode}">SuperJSON</code>.
				</p>
				<spartan-code class="mt-3" language="ts" code="
				// trpc-client.ts
...
service_role key: eyJh......"
			...
			<p class="${hlmP}">
				You can setup your schema manually by running sql-query in the SQL-Editor locally or in supabase. However, you can also utilize <code class="${hlmCode}">drizzle-kit</code> to easily
				<code class="${hlmCode}">generate</code>, <code class="${hlmCode}">migrate</code>, <code class="${hlmCode}">push</code>, <code class="${hlmCode}">pull</code>, and more.
				Refer to official <a class="underline" href="https://orm.drizzle.team/docs/kit-overview">Drizzle Documentation</a>.
			</p>
			<p class="${hlmP}">
				To get started with drizzle-kit. Let's install it first.
			</p>
			...
			<p class="${hlmP}">
				...
				<code class="${hlmCode}">[YOUR_APP_NAME]/drizzle.config.ts</code> You can also create multiple config file for multiple environment <code class="${hlmCode}">[YOUR_APP_NAME]/drizzle-[ENV].config.ts</code>.
...
export default defineConfig({
  ...
  dbCredentials: {
    url: process.env.DATABASE_URL!,
			...
			/>
			<p class="${hlmP}">
				Now you can use the power of drizzle-kit and generate SQL migrations based on your Drizzle schema, migrate them and push them to your supabase.
				Run the following command to generate the initial sql migrations. 
			</p>
			<spartan-code class="mt-3" language="sh" code="npx drizzle-kit generate --name=init" />
			<p class="${hlmP}">
				Once generated, you can find your migrations sql script at <code class="${hlmCode}">[YOUR_APP_NAME]/src/drizzle/migrations</code>
			</p>
			<p class="${hlmP}">
				Run the following command to migrate the generated migrations.
			</p>
			<spartan-code class="mt-3" language="sh" code="npm i -D drizzle-kit migrate" />
			<p class="${hlmP}">
				Finally you can push your migrations using the following command.
			</p>
			<spartan-code class="mt-3" language="sh" code="npm i -D drizzle-kit push" />
			<p class="${hlmP}">
				If you have multiple drizzle config then, you can pass <code class="${hlmCode}">--config=drizzle-[env].config.ts</code> to any of the drizzle-kit commands.
				For example, you would run the following command to <code class="${hlmCode}">generate</code> migrations.
			</p>
			<spartan-code class="mt-3" language="sh" code="npm i -D drizzle-kit generate --name=init --config=drizzle-[env].config.ts" />
			<p class="${hlmP}">
...
"
			...
			<p class="${hlmP}">
				You can find the final repository for this installation <a class="underline hover:cursor-pointer" href="https://github.com/Rockerturner/spartan-drizzle">here</a>.
			</p>
