...
export class BrnQuestionnairePrevious {
	private readonly _questionnaire = injectBrnQuestionnaire();

	public readonly disabled = input<boolean, BooleanInput>(false, { transform: booleanAttribute });

	public readonly visible = computed(() => this._questionnaire.total() > 1 && !this._questionnaire.first());
	...
	protected onClick(event: Event): void {
		...
		this._questionnaire.goPrevious();
	}
}
