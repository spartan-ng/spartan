...
import { getShortcutKeys } from './brn-questionnaire.utils';

export type BrnQuestionnaireCollection = {
	enabledItems: readonly BrnQuestionnaireItemDefinition[];
	itemByName: ReadonlyMap<string, BrnQuestionnaireItemDefinition>;
	items: readonly BrnQuestionnaireItemDefinition[];
};

export function createQuestionnaireCollection(
	items: readonly BrnQuestionnaireItemDefinition[] | undefined,
): BrnQuestionnaireCollection | null {
	if (items === undefined) {
		return null;
	}
...
export function getInitialItemName(
	collection: BrnQuestionnaireCollection | null,
...
export function getShortcutByChoiceValue(
	item: BrnQuestionnaireItemDefinition | undefined,
	shortcuts: BrnQuestionnaireShortcutMode | null,
): Map<string, string> {
...
export function getCollectionDefinitionWarnings(
	collection: BrnQuestionnaireCollection,
	defaultItem: string | undefined,
): string[] {
	...
	for (const item of collection.items) {
		...
		for (const choice of item.choices ?? []) {
			if (choiceValues.has(choice.value)) {
				warnings.push(`Choice value "${choice.value}" is defined more than once in item "${item.name}".`);
			}
...
export function getCollectionRegistrationWarnings(
	collection: BrnQuestionnaireCollection,
	registrations: readonly BrnItemRegistration[],
	shortcuts: BrnQuestionnaireShortcutMode | null,
): string[] {
	const warnings: string[] = [];
	const registrationByName = new Map(registrations.map((registration) => [registration.name, registration]));
	...
	for (const definition of collection.items) {
		...
		const registeredChoiceByValue = new Map(registration.choices.map((choice) => [choice.value, choice]));
		...
		if (shortcuts) {
			...
			const registeredOrder = registration.choices.filter((choice) => !choice.disabled).map((choice) => choice.value);
			...
			if (sameChoices && definedOrder.some((choiceValue, index) => choiceValue !== registeredOrder[index])) {
				warnings.push(
