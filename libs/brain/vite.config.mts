...
export default defineConfig(({ mode }) => ({
	...
	test: {
		...
		include: ['**/*.spec.ts'],
		reporters: ['default'],
		// A few sonner specs assert real-timer toast lifetimes (durations of seconds). Running test
		// files in parallel browser contexts starves those timers of CPU and skews the windows, so
		// serialize file execution; the timing specs dominate runtime regardless of parallelism.
		fileParallelism: false,
		// Real-browser testing via Playwright/Chromium - jsdom cannot faithfully exercise layout,
		// focus, pointer events or computed styles that these primitive specs rely on.
		browser: {
			enabled: true,
			headless: true,
			provider: 'playwright',
	...
	cacheDir: '../../node_modules/.vitest/brain',
}));
