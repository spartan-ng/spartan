...
`);
		...
		it('does not backslash-escape quotes in attribute selectors (issue #1572)', async () => {
			// Escaped quotes ([class*=\'text-\']) don't match the unescaped DOM class, so Tailwind
			// emits no CSS (#1572).
			const source = `const cls = hlm('spartan-button');`;

			const result = await applyTransform(source, baseStyleMap);

			expect(result).toContain("[&_ng-icon:not([class*='text-'])]:text-base");
