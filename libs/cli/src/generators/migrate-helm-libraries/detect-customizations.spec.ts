...
describe('detect-customizations', () => {
	...
	describe('getMetadata and saveMetadata', () => {
		...
		it('should prune malformed library entries but keep valid ones', () => {
			tree.write(
				'.spartan/library-metadata.json',
				JSON.stringify({
					libraries: {
						button: { version: '1.0.0', timestamp: 1, files: { 'src/index.ts': 'hash' } },
						card: { version: '1.0.0' },
					},
				}),
			);
			...
			expect(metadata.libraries.card).toBeUndefined();
		});
	...
	describe('updateLibraryMetadata', () => {
		...
		it('should preserve other libraries metadata when updating one', () => {
			...
			const metadata2 = getMetadata(tree);
			expect(metadata2.libraries.card).toEqual(metadata1.libraries.card);
			expect(metadata2.libraries.button.version).toBe('2.0.0');
		});
	});
