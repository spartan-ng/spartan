...
export class HlmComboboxItem {
	...
	constructor() {
		classes(
			() =>
				`data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground relative flex w-full cursor-default items-center gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm outline-hidden select-none data-hidden:hidden data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_[hlmIcon]]:pointer-events-none [&_[hlmIcon]]:shrink-0 [&_[hlmIcon]:not([class*='text-'])]:text-base`,
		);
	}
}
