...
import { HlmCalendarRange } from '../../../calendar/src/lib/hlm-calendar-range';
import { injectHlmDateRangePickerConfig } from './hlm-date-range-picker.token';

export type HlmDateRange<T> = {
	start: T | undefined;
	end: T | undefined;
};
...
export class HlmDateRangePicker<T> implements ControlValueAccessor {
	...
	public readonly date = model<HlmDateRange<T>>({
		start: undefined,
		end: undefined,
	});

	/** If true, the date picker will close when the end date is selected */
	public readonly autoCloseOnMaxSelection = input<boolean, BooleanInput>(this._config.autoCloseOnEndSelection, {
		transform: booleanAttribute,
	});
	...
	writeValue(value: HlmDateRange<T> | null): void {
