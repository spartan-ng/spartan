...
export class HlmInputGroup {
	...
	protected readonly _wrapperClasses = computed(() =>
		hlm(
			'group flex w-full items-stretch rounded-md shadow-xs has-[*:disabled]:pointer-events-none has-[*:disabled]:opacity-50 ' +
				// Reset inner input/textarea
				'[&_[hlmInput]]:h-auto [&_[hlmInput]]:flex-1 [&_[hlmInput]]:border-0 [&_[hlmInput]]:bg-transparent [&_[hlmInput]]:px-0 [&_[hlmInput]]:py-0 [&_[hlmInput]]:shadow-none [&_[hlmInput]]:ring-0 [&_[hlmInput]]:ring-offset-0 [&_[hlmInput]]:outline-none [&_textarea[hlmInput]]:min-h-6' +
				this.userClass(),
		),
	);
	...
	protected readonly _inputWrapperClasses = computed(() =>
		hlm(
			'border-input bg-background placeholder:text-muted-foreground focus-within:border-ring focus-within:ring-ring/50 z-20 flex h-auto min-h-9 w-full flex-row gap-1 rounded-md border px-3 text-sm transition-colors file:border-0 file:bg-transparent file:text-sm file:font-medium focus-within:ring-[3px] focus-within:outline-none' +
				...
				' group-has-[hlm-prefix-addon,[hlmPrefixAddon]]:rounded-l-none group-has-[hlm-prefix-addon,[hlmPrefixAddon]]:border-l-0 group-has-[hlm-suffix-addon,[hlmSuffixAddon]]:rounded-r-none group-has-[hlm-suffix-addon,[hlmSuffixAddon]]:border-r-0',
