...
export function registerComponentTools(server: McpServer): void {
	...
	server.registerTool(
		'spartan_components_get',
		{
			...
			inputSchema: {
				name: z.string().min(1, 'name is required').describe("Component name (kebab-case), e.g., 'accordion'."),
				extract: z
					.enum(['code', 'api'])
					.default('code')
					.describe("Extraction: 'code' for code blocks, 'api' for structured API information."),
		...
		async (args) => {
			...
			if (!noCache) {
				...
				if (cached.cached && !cached.stale && isComponentSnapshot(cached.data)) {
