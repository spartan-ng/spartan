...
import { booleanAttribute, Directive, ElementRef, inject, input, output, PLATFORM_ID, signal, afterNextRender } from '@angular/core';
...
export class BrnAutocompleteItem<T> implements Highlightable {
    ...
    /** @internal Get the display value (guarded for SSR) */
    ...
    setActiveStyles(): void {
        ...
        // ensure the item is in view — defer to afterNextRender and guard for SSR
        if (!isPlatformBrowser(this._platform)) return;
        afterNextRender(() => {
            try {
                const el = this._elementRef.nativeElement as HTMLElement | null;
